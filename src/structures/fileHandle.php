...
class fileUpload
{
    ...
    private function fileSize()
    {
        if ($this->srcFile['size'] > $this->sizeCap) {
            send_error_response("File size too large", 413);
        }
    }
    ...
    private function ext_Chk()
    {
        ...
        switch ($contType) {
            ...
            case "application":
                ...
                break;
            default:
                $extArray = ["jpeg", "jpg", "png", "bmp", "webp", "json"];
        }
        ...
        // print_r("here is real ext " . $realExt);
        if (!in_array($realExt, $extArray)) {
            send_error_response("Invalid file type", 400);
        }
    }
